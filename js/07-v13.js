...
(function(){
  ...
  window.go = function(id, el){
    ...
    if(v13panes.indexOf(id)>-1){
      ...
      var titles={devis:'Devis',bc:'Bons de commande',bl:'Bons de livraison',recurrentes:'Factures récurrentes',inventaire:'Inventaire physique',provisions:'Provisions & Dépréciations',multidevises:'Multi-devises',tft:'Tableau des Flux de Trésorerie',calendrier:'Calendrier fiscal',profil:'Profil entreprise'};
...
function renderDevis(){
  ...
  DEVIS_LIST.slice().reverse().forEach(function(dv,i){
    ...
    html+='<tr style="border-bottom:2px solid var(--border)"><td style="padding:8px 10px;font-family:\'Archivo\',sans-serif;font-weight:700;color:var(--green)">'+dv.num+'</td><td style="padding:8px 10px">'+dv.client+'</td><td style="padding:8px 10px;max-width:180px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap" title="'+dv.objet+'">'+dv.objet+'</td><td style="padding:8px 10px;text-align:right;font-family:\'Archivo\',sans-serif;font-weight:600">'+dv.ttc.toLocaleString('fr-FR')+'</td><td style="padding:8px 10px;font-size:11px;color:var(--text-muted)">'+fmtD(dv.validite)+'</td><td style="padding:8px 10px">'+badge(dv.statut,(statusColors[dv.statut]||'bg-blue'))+'</td><td style="padding:8px 10px"><div style="display:flex;gap:4px;flex-wrap:wrap"><button class="btn btn-sm" style="font-size:9px;padding:2px 6px" onclick="changerStatutDevis('+idx+')" title="Changer statut">'+ico('pencil')+'</button>'+(dv.statut==='accepte'?'<button class="btn btn-sm" style="font-size:9px;padding:2px 6px;background:var(--amber);color:#fff;border-color:var(--amber)" onclick="convertirDevisEnBC('+idx+')">→ BC</button>':'')+'<button class="btn btn-sm" style="font-size:9px;padding:2px 6px;background:var(--blue);color:#fff;border-color:var(--blue)" onclick="imprimerDevis('+idx+')">'+ico('printer')+'</button><button class="btn btn-sm" style="font-size:9px;padding:2px 6px;color:var(--red);border-color:var(--red-border)" onclick="supprimerDevis('+idx+')">'+ico('close')+'</button></div></td></tr>';
  });
...
function renderBL(){
  ...
  BL_LIST.slice().reverse().forEach(function(bl,i){
    ...
    html+='<tr style="border-bottom:2px solid var(--border)"><td style="padding:8px 10px;font-family:\'Archivo\',sans-serif;font-weight:700;color:var(--blue)">'+bl.num+'</td><td style="padding:8px 10px">'+bl.client+'</td><td style="padding:8px 10px;max-width:180px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">'+bl.desc+'</td><td style="padding:8px 10px;font-size:11px;color:var(--text-muted)">'+fmtD(bl.date)+'</td><td style="padding:8px 10px;font-family:\'Archivo\',sans-serif;font-size:11px">'+( bl.bcRef||'—')+'</td><td style="padding:8px 10px"><div style="display:flex;gap:4px"><button class="btn btn-sm" style="font-size:9px;padding:2px 6px;background:var(--blue);color:#fff;border-color:var(--blue)" onclick="imprimerBL('+idx+')">'+ico('printer')+' PDF</button><button class="btn btn-sm" style="font-size:9px;padding:2px 6px;color:var(--red);border-color:var(--red-border)" onclick="supprimerBL('+idx+')">'+ico('close')+'</button></div></td></tr>';
  });
  html+='</tbody></table></div></div>';
  w.innerHTML=html;
}
...
function comptabiliserEcarts(){
  ...
  if(summary){summary.style.display='block';summary.innerHTML='<div style="padding:10px 14px;background:var(--green-light);border:2px solid var(--green-border);border-radius:var(--radius);color:var(--green-dark);font-size:12px">'+ico('check')+' <strong>'+ecritures.length+' écritures d\'inventaire comptabilisées</strong> dans le journal (D.6031/C.3111 pour les manques, D.3111/C.7032 pour les surplus). Vérifiez le journal.</div>';}
...
function calculerTFT(){
  ...
  html+=renderSection('Flux d\'exploitation',ico('gear'),detailsExp,fluxExp,'green');
  html+=renderSection('Flux d\'investissement',ico('factory'),detailsInv,fluxInv,'blue');
  html+=renderSection('Flux de financement',ico('briefcase'),detailsFin,fluxFin,'purple');
...
function renderCalendrier(){
  ...
  if(pEl&&prochaine){
    var urgence=prochaine.joursRestants<=7?'<span style="color:var(--red)">'+ico('dot')+'</span>':prochaine.joursRestants<=30?'<span style="color:var(--amber)">'+ico('dot')+'</span>':'<span style="color:var(--green)">'+ico('dot')+'</span>';
...
(function(){
  ...
  var newTitles={devis:'Devis',bc:'Bons de commande',bl:'Bons de livraison',recurrentes:'Factures récurrentes',inventaire:'Inventaire physique',provisions:'Provisions & Dépréciations',multidevises:'Multi-devises',tft:'Tableau des Flux de Trésorerie',calendrier:'Calendrier fiscal',profil:'Profil entreprise'};
