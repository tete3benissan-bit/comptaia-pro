...
function renderBilanV9(){
  ...
  if(diff<100){eq.style.cssText='display:block;margin-top:10px;padding:9px 14px;border-radius:var(--radius);font-size:12px;font-weight:600;text-align:center;background:var(--green-light);color:var(--green-dark);border:2px solid var(--green-border)';eq.innerHTML='✓ Bilan OHADA équilibré — '+Math.round(tActif).toLocaleString('fr-FR')+' FCFA<br><span style="font-size:10px;font-weight:400">Actif immobilisé : '+Math.round(tActifImmo).toLocaleString('fr-FR')+' | Actif circulant : '+Math.round(tActifCirc).toLocaleString('fr-FR')+'</span>';}
  else{eq.style.cssText='display:block;margin-top:10px;padding:9px 14px;border-radius:var(--radius);font-size:12px;font-weight:600;text-align:center;background:var(--amber-light);color:var(--amber);border:2px solid var(--amber-border)';eq.innerHTML='⚠ Écart de '+Math.round(diff).toLocaleString('fr-FR')+' FCFA — Vérifiez capital social (cpt 101) et dettes (cpt 16)<br><span style="font-size:10px;font-weight:400">Actif : '+Math.round(tActif).toLocaleString('fr-FR')+' / Passif : '+Math.round(tPassif).toLocaleString('fr-FR')+'</span>';}}
}
